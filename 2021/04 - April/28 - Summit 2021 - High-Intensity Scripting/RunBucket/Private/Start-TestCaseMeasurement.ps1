function Start-TestCaseMeasurement {
    ...
    $Code = {
        ...
        1..$_Iterations | ForEach-Object {[PSCustomObject] @{Iteration = $_; Script = $_ScriptBlock}} |
            Start-RSJob -ScriptBlock $JobBlock -Name {$_.Iteration} -Throttle $_Throttle |
            Wait-RSJob
        $RunTime = (Receive-RSJob -Name $_Iterations).RunTime | Sort-Object
        $RunTime[1..($RunTime.Length-2)] | Measure-Object -Average -Maximum -Minimum
    }

    $Result = if ($UseWinPS) {
        powershell.exe -Command $Code -args $ScriptBlock, $Iterations, $Throttle
    } else {
        Invoke-Command -ScriptBlock $Code -ArgumentList $ScriptBlock, $Iterations, $Throttle
    }
